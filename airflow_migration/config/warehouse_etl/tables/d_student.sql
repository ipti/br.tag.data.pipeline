...
LEFT JOIN {database_name}.student_documents_and_address sd ON si.id = sd.id
LEFT JOIN {database_name}.school_identification si2 ON si.school_inep_id_fk = si2.inep_id
LEFT JOIN {database_name}.edcenso_city ec ON si.edcenso_city_fk = ec.id
LEFT JOIN {database_name}.edcenso_uf eu ON si.edcenso_uf_fk = eu.id
