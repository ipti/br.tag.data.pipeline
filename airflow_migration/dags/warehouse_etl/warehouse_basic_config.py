...
class TriggerConfig:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.schedule_interval, str) or not self.schedule_interval.strip():
            ...
            logger.error("Invalid trigger configuration", extra_data={
        ...
        logger.debug("TriggerConfig validated successfully", extra_data={
            "trigger_name": self.name,
            "schedule_interval": self.schedule_interval
        })
...
class TableLoad:
    ...
    def __post_init__(self):
        ...
        if not self.table_name or not isinstance(self.table_name, str):
            ...
            logger.error("Invalid TableLoad configuration", extra_data={
        ...
        logger.debug("TableLoad validated successfully", extra_data={
            "table_name": self.table_name,
            "model": self.model,
            "trigger": self.trigger,
...
class Stage:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.stage, int) or self.stage < 1:
            ...
            logger.error("Invalid Stage configuration", extra_data={
                "stage": self.stage,
        ...
        logger.debug("Stage validated successfully", extra_data={
            "stage": self.stage,
...
class WorkflowConfig:
    ...
    def __post_init__(self):
        ...
        if not self.workflow_name or not isinstance(self.workflow_name, str):
            ...
            logger.error("Invalid WorkflowConfig", extra_data={
        ...
        if not isinstance(self.stages, list) or not self.stages:
            ...
            logger.error("Invalid WorkflowConfig", extra_data={
                ...
                "stages_length": len(self.stages) if isinstance(self.stages, list) else 0,
                "error": error_msg
            })
            raise ValueError(error_msg)
        ...
        for trigger_name, trigger_config in self.triggers.items():
            if not isinstance(trigger_config, TriggerConfig):
                error_msg = f"Trigger '{trigger_name}' must be an instance of TriggerConfig"
                logger.error("Invalid WorkflowConfig", extra_data={
                    "workflow_name": self.workflow_name,
        ...
        logger.info("WorkflowConfig validated successfully", {
            "workflow_name": self.workflow_name,
            "max_parallel_tasks": self.max_parallel_tasks,
...
class TableConfig:
    ...
    def __post_init__(self):
        ...
        if not self.table_name:
            ...
            logger.error("Invalid TableConfig", extra_data={
                "table_name": self.table_name,
                "error": error_msg
            })
            raise ValueError(error_msg)
        ...
        logger.debug("TableConfig validated successfully", extra_data={
            "table_name": self.table_name,
            "sql_path": self.sql_path,
    ...
    def __post_init__(self):
        ...
        for field_name, field_value in required_fields.items():
            if not field_value or not isinstance(field_value, str):
                ...
                logger.error("Invalid FilterCombination", extra_data={
        ...
        logger.debug("FilterCombination validated successfully", extra_data={
            "database_name": self.database_name,
            "school_inep_fk": self.school_inep_fk,
            "classroom_id": self.classroom_id,
