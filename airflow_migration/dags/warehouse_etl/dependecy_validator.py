    ...
    def __post_init__(self):
        ...
        if not self.message:
            if self.error_type == "missing_table":
                ...
                cycle_str = " -> ".join(self.cycle_path) if self.cycle_path else "unknown"
...
class DependencyValidator:
    ...
    def validate_dependencies(self, workflow_config: WorkflowConfig) -> Tuple[bool, List[DependencyError]]:
        """
        ...
        errors.extend(self._validate_stage_dependencies(workflow_config, table_to_stage, all_tables))
        ...
        if is_valid:
            ...
            self.logger.error("Dependency validation failed", extra_data={
                "workflow_name": workflow_config.workflow_name,
    ...
    def _build_table_mappings(self, workflow_config: WorkflowConfig) -> Tuple[Dict[str, int], Set[str]]:
        """
        ...
        for stage in workflow_config.stages:
            for table_load in stage.loads:
                ...
                if table_name in table_to_stage:
                    self.logger.warning("Duplicate table name found", extra_data={
        ...
        self.logger.debug("Table mappings built", extra_data={
            "total_tables": len(all_tables),
    ...
    def _validate_stage_dependencies(
        ...
        self.logger.debug("Stage dependency validation completed", extra_data={
    ...
    def _detect_circular_dependencies(self, workflow_config: WorkflowConfig, all_tables: Set[str]) -> List[DependencyError]:
        """
        ...
        self.logger.debug("Circular dependency detection completed", extra_data={
    ...
    def _build_dependency_graph(self, workflow_config: WorkflowConfig) -> Dict[str, List[str]]:
        """
        ...
        self.logger.debug("Dependency graph built", extra_data={
    ...
    def get_dependency_summary(self, workflow_config: WorkflowConfig) -> Dict[str, any]:
        ...
        max_dependencies = max((len(deps) for deps in dependency_graph.values()), default=0)
        
        root_tables = [table for table in all_tables if not dependency_graph.get(table, [])]
