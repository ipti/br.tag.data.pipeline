...
def get_classroom_mapping():
    try:
        ...
        turmas = [
            f'Turma 9o Ano {np.random.choice(["A", "B", "C"])}' for _ in range(2000)
        ]
        escolas = [
            f'Escola {np.random.choice(["Alvorada", "Sertaozinho", "Liberdade", "Esperanca"])}'
            for _ in range(2000)
        ]
...
y_matematico = (
    (X["nota_mat_norm"] * 0.3)
    + (X["nota_lp_norm"] * 0.25)
    + (X["nota_ciencias_norm"] * 0.15)
    - (X["pct_disciplinas_abaixo5"] * 2.5)
    - (X["taxa_ausencia_pct"] * 0.05)
)
...
painel_turmas = (
    df_merged.groupby(["school", "classroom"])
    .agg(
        media_prevista=("pred_nota_final", "mean"), total_alunos=("student_id", "count")
    )
    .reset_index()
)
...
df_alunos_isolados = df_merged[
    (df_merged["school"] == alvo_escola) & (df_merged["classroom"] == alvo_turma)
]
X_isolado = df_alunos_isolados.drop(
    columns=["student_id", "pred_nota_final", "school", "classroom"]
)
...
diagnostico_df = pd.DataFrame(
    {
        ...
        "Impacto Matemático na Nota GERAL da Turma (Pontos)": np.round(
            impacto_na_media_geral, 2
        ),
    }
...
diagnostico_df = (
    ...
    .sort_values("Impacto (SHAP Value)", ascending=True)
    .head(5)
)
