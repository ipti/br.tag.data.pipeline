...
def _refresh_all_embeddings():
    ...
    logger.info(
        "Buscando dados de alunos para embedding (todos os anos disponíveis)..."
    )
    ...
    if not df_students.empty:
        logger.info(
            "Verificando atualizações incrementais em %d Estudantes.", len(df_students)
        )
        ...
        df_needs_update = df_students[
            df_students["text_hash"] != df_students["existing_hash"]
        ].copy()

        logger.info(
            "%d Alunos necessitam atualização de embedding (o restante será ignorado).",
            len(df_needs_update),
        )
        ...
        if not df_needs_update.empty:
            ...
            for start in range(0, total, CHUNK_SIZE):
                ...
                logger.info(
                    "Encoding chunk [%d:%d]/%d", start, start + len(chunk), total
                )
    ...
    try:
        ...
        df_schools = (
            compute_school_metrics(df_schools_raw)
            if not df_schools_raw.empty
            else pd.DataFrame()
        )
    ...
    if not df_schools.empty:
        logger.info(
            "Verificando atualizações incrementais em %d Escolas.", len(df_schools)
        )
        ...
        school_hashes = [
            hashlib.md5(t.encode("utf-8")).hexdigest() for t in school_texts
        ]

        df_schools["text_hash"] = school_hashes
        df_schools["existing_hash"] = df_schools["school_id"].map(
            school_existing_hashes
        )
        df_schools_update = df_schools[
            df_schools["text_hash"] != df_schools["existing_hash"]
        ].copy()

        logger.info(
            "%d Escolas necessitam atualização de embedding.", len(df_schools_update)
        )
