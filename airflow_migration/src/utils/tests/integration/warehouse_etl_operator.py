...
def sample_table_execution_dict(tmp_path: Path) -> dict:
    ...
    inc_config = IncrementalConfig(source_timestamp_columns=["updated_at"], target_timestamp_column="inserted_at")
    ups_config = UpsertConfig(source_key_columns=["id"])
    db_name = "nossasenhoradagloria_db"

    exec_obj = TableExecution(
        table_name="d_student", stage=1, database=db_name, source_name="{SOURCE_NAME}",
        target_schema="{TARGET_SCHEMA}", trigger="daily", model="tables/d_student.yml",
        sql_path=str(sql_file), incremental_config=inc_config, upsert_config=ups_config,
...
def test_operator_execute_success_flow(mock_copy_loader_class, mock_db_manager_class, sample_table_execution_dict):
    """
    ...
    mock_db_manager_instance.is_production, mock_db_manager_instance.hotfix_mode = False, False
    ...
    mock_loader_instance.incremental_load.return_value = LoadResult(success=True, rows_processed=100)
    
    operator = WarehouseEtlOperator(task_id="test_success", table_execution_dict=sample_table_execution_dict)
    ...
    assert "SELECT * FROM `nossasenhoradagloria_db`.raw_students" in call_kwargs['source_query']
    assert result == {"status": "success", "rows_processed": 100}

@patch('plugins.operators.warehouse_etl_operator.DatabaseConnectionManager')
@patch('plugins.operators.warehouse_etl_operator.CopyAndLoader')
def test_operator_handles_load_failure(mock_copy_loader_class, mock_db_manager_class, sample_table_execution_dict):
    """
    ...
    """
    mock_loader_instance = mock_copy_loader_class.return_value
    mock_loader_instance.incremental_load.return_value = LoadResult(success=False, error_message="DB timeout")
    
    operator = WarehouseEtlOperator(task_id="test_failure", table_execution_dict=sample_table_execution_dict)
    with pytest.raises(AirflowException) as excinfo:
        operator.execute(context={})
    
    assert "Load operation failed for table d_student. Error: DB timeout" in str(excinfo.value)

@patch('plugins.operators.warehouse_etl_operator.DatabaseConnectionManager')
@patch('plugins.operators.warehouse_etl_operator.CopyAndLoader')
def test_operator_hotfix_mode_is_passed(mock_copy_loader_class, mock_db_manager_class, sample_table_execution_dict):
    """
    ...
    """
    operator = WarehouseEtlOperator(task_id="test_hotfix", table_execution_dict=sample_table_execution_dict, hotfix=True)
    operator.execute(context={})
...
def test_operator_fails_on_missing_sql_file(sample_table_execution_dict):
    ...
    operator = WarehouseEtlOperator(task_id="test_missing_file", table_execution_dict=sample_table_execution_dict)
    with pytest.raises(AirflowException) as excinfo:
        operator.execute(context={})
