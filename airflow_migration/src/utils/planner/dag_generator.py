...
"""
        if (
            trigger_config.schedule_interval in [None, "", "None", "null"]
            or not trigger_config.schedule_interval
        ):
            schedule_code = "None"
        else:
            schedule_code = f'"{trigger_config.schedule_interval}"'
...
"""
    ...
    with TaskGroup(group_id="Batch_{i + 1}_Stage_{stage_num}") as {tg_var}:"""
        ...
        if trigger_config.trigger_dag_on_success:
            triggered_trigger_name = trigger_config.trigger_dag_on_success

            triggered_dag_id = f"{workflow_config.workflow_name.lower()}__{triggered_trigger_name}__{environment}"
            sanitized_task_var = (
