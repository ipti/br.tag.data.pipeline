...
def transform_enrollment_student(row):
    ...
    return {
        **row,
        "student_id": (
            str(row["student_id"]).split("-")[0] if row.get("student_id") else None
        ),
        "classroom_id": (
            str(row["classroom_id"]).split("-")[0] if row.get("classroom_id") else None
        ),
    }
...
def transform_school_geograph_rel(row):
    ...
    return {
        ...
        "school_id": (
            str(row.get("geograph_id", "")).split("-")[0]
            if row.get("geograph_id")
            else None
        ),
        "inserted_at": row.get("inserted_at"),
    }
...
def transform_enrollment_school(row):
    ...
    return {
        ...
        "school_id": (
            str(row.get("school_id")).split("-")[0] if row.get("school_id") else None
        ),
...
def transform_enrollment_health(row):
    ...
    return {
        ...
        "health_id": (
            str(row["health_id"]).split("-")[0] if row.get("health_id") else None
        ),
        "inserted_at": row.get("inserted_at"),
    }
...
def transform_class_school(row):
    ...
    return {
        "class_id": row.get("class_id"),
        "school_id": (
            str(row["school_id"]).split("-")[0] if row.get("school_id") else None
        ),
        "updated_at": row.get("updated_at"),
    }
