...
from utils.connections.connection_manager import DatabaseConnectionManager, DatabaseConfig, get_db_manager
...
class TestDatabaseConnectionManagerUnit:
    ...
    @patch('utils.connections.connection_manager.subprocess.run')
    @patch('utils.connections.connection_manager.get_logger')
    def test_mysql_host_resolution(self, mock_logger, mock_subprocess, mock_gethostbyname):
    ...
    def test_multiple_mysql_sources_discovery(self, mock_logger, mock_subprocess):
        ...
        print(f"Found {len(manager.mysql_configs)} MySQL sources: {list(manager.mysql_configs.keys())}")
    ...
    def test_mysql_connection_health_check(self, db_manager):
        ...
        for source_name in db_manager.mysql_configs.keys():
            try:
                ...
                print(f"MySQL {source_name} health check: {'✅' if is_healthy else '❌'}")
            except Exception as e:
...
def manual_test_runner():
    ...
    try:
        ...
        logger.info("Environment detection completed", extra_data={
        ...
        for host in test_hosts:
            ...
            logger.info(f"Host resolution test", extra_data={
                "original_host": host,
        ...
        if db_manager.mysql_configs:
            for source_name in db_manager.mysql_configs.keys():
                try:
                    is_healthy = db_manager.test_mysql_connection(source_name)
                    logger.info("MySQL connection test completed", extra_data={
                        "source": source_name,
                        "status": "healthy" if is_healthy else "unhealthy"
                    })
                except Exception as e:
                    logger.error(f"MySQL connection test failed", exception=e, extra_data={
                        "source": source_name
                    })
        else:
        ...
        try:
            is_healthy = db_manager.test_sqlserver_connection()
            logger.info("SQL Server connection test completed", extra_data={
                "status": "healthy" if is_healthy else "unhealthy"
            })
        except Exception as e:
